...
public class PinkPawnsView implements PropertyChangeListener{
    VBox pawnsCtn;
    Label crossPawnsCount;
    Label circlePawnsCount;
    ImageView crossPawnsImage = new ImageView(Images.PINK_CROSS_PAWN);
    ImageView circlePawnsImage = new ImageView(Images.PINK_CIRCLE_PAWN);
    Image boardImage =  Images.WHITE_LEFT_PAWNS;
    double screenHeight = Toolkit.getDefaultToolkit().getScreenSize().getHeight();
    private GameFacade gameFacade;
    BackgroundSize backgroundSize = new BackgroundSize(screenHeight/4, screenHeight/2,false,false,false, false);
    public PinkPawnsView(){
        this.crossPawnsCount = new Label();
        this.circlePawnsCount = new Label();
        this.pawnsCtn = new VBox();
    }
    public PinkPawnsView(GameFacade gameFacade){
        this.crossPawnsCount = new Label();
        this.circlePawnsCount = new Label();
        this.pawnsCtn = new VBox();
    ...
    public void setup(HBox gamePane, int initialCrossPawnsCount, int initialCirclePawnsCount){
        ...
        this.circlePawnsCount.setText(Integer.toString(initialCrossPawnsCount));
    ...
    public void update(int crossPawnsCount, int circlePawnsCount, PieceColor playerColor){
        ...
        if(playerColor == null){
            this.boardImage = Images.WHITE_LEFT_PAWNS;
        }else if(playerColor.equals(PieceColor.BLACK)){
            this.boardImage = Images.WHITE_LEFT_PAWNS;
        } else {
            this.boardImage = Images.PINK_LEFT_PAWNS;
        }
        BackgroundImage backgroundImage = new BackgroundImage(boardImage,
                BackgroundRepeat.NO_REPEAT,
                BackgroundRepeat.NO_REPEAT,
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        this.update(this.gameFacade.getPinkCrossPawnsCount(), this.gameFacade.getPinkCirclePawnsCount(), null);
    }
}
